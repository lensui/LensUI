...
import { createCellFont, getCellBaselineOffset } from './typography';
...
export function paintGrid<Row extends object>(options: PaintOptions<Row>): void {
  ...
  for (let columnIndex = 0; columnIndex < columns.length; columnIndex += 1) {
    ...
    ctx.fillStyle = colors.grid;
    if (!verticalBorderless && !(frameBorderless && columnIndex === columns.length - 1)) {
      ctx.fillRect(x + metric.width - 1, headerY, 1, headerHeight);
    }
  ...
  ctx.fillStyle = colors.grid;
  if (!horizontalBorderless && !frameBorderless) ctx.fillRect(0, 0, width, 1);
  if (!verticalBorderless && !frameBorderless) {
    ctx.fillRect(0, 0, 1, height);
    ctx.fillRect(width - 1, 0, 1, height);
  }
  if (!horizontalBorderless && !frameBorderless && !options.suppressFrameBottomBorder) ctx.fillRect(0, height - 1, width, 1);
